...
describe('Copy race drivers', () => {
    ...
    beforeEach(async () => {
        ...
        await adminFirestore.doc(`${seasonsURL}/9999/races/${round1.round}`).set({ ...round1, state: 'waiting' });
    ...
    it('opening first round should not result in an error', async () => {
        await assertSucceeds(updateRace(round1, {state: 'open'}))
            .then(() => new Promise(resolve => setTimeout(() => resolve(), 2000)))
            .then(() => readRace(round2.round))
            .then((race: IRace) => expect(race.drivers).toBeUndefined())
            .catch((error: any) => fail(error))
    })    

    it('opening second round should result in round 2 has all drivers from round 1', async () => {
        await updateRace(round1, {state: 'completed'});
        await assertSucceeds(updateRace(round2, {state: 'open'}))
            .then(() => new Promise(resolve => setTimeout(() => resolve(), 2000)))
            .then(() => readRace(round2.round))
            .then((race: IRace) => expect(race.drivers).toStrictEqual(round1.drivers))
            .catch((error: any) => fail(error))
    })
