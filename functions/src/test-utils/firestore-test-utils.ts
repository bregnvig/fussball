...
export async function authedApp(auth?: object): Promise<AuthedAppData> {
  // await firebaseTesting.clearFirestoreData({ projectId: 'fussball-dev'})
  const app = firebaseTesting.initializeTestApp({ projectId: 'fussball-dev', auth });
...
export function adminApp() {
  const firestore = firebaseTesting.initializeAdminApp({ projectId: 'fussball-dev' }).firestore();
  return firestore;
}
